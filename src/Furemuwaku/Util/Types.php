...
enum Types: Int
{
	...
	public function valid( Mixed $value, ? Types $self = Null ): Bool
	{
		return( match( $self ?? $this )
		{
			...
			self::ARRAY_ASSOCIATIVE => is_array( $value ) && Arr::isAssoc( $value ),
			self::ARRAY_LIST => is_array( $value ) && Arr::isList( $value ),
			self::ARRAY_MULTIDIMENSION => is_array( $value ) && Arr::isMulti( $value ),
			self::BINARY,
			self::INTEGER_BINARY => Number::isBinary( $value ),
			...
			self::DECIMAL,
			self::INTEGER_DECIMAL => Number::isDecimal( $value ),
			self::DOUBLE,
			self::NUMERIC_DOUBLE => is_double( $value ) || Number::isDouble( $value ),
			self::EMAIL => Str::isEmail( $value ),
			self::EXPONENT_DOUBLE,
			self::NUMERIC_EXPONENT_DOUBLE => Number::isExponentDouble( $value ),
			self::FLOAT,
			self::NUMERIC_FLOAT => is_float( $value ) || Number::isFloat( $value ),
			self::FINITE => is_float( $value ) && is_finite( $value ),
			self::INFINITE => is_float( $value ) && is_infinite( $value ),
			self::HEXA,
			self::INTEGER_HEXA => Number::isHexa( $value ),
			self::INT,
			self::NUMERIC_INT => is_int( $value ) || Number::isInt( $value ),
			self::INTEGER => is_integer( $value ) || Number::isInteger( $value ),
			self::IP => is_string( $value ) && IP\IPAddress::valid( $value ),
			self::JSON => Str::isJson( $value ),
			self::LONG,
			self::MIXED => True,
			self::NUMERIC_LONG => is_long( $value ) || Number::isLong( $value ),
			self::NAN => is_nan( $value ),
			self::NUMBER,
			self::NUMERIC_NUMBER => Number::isNumber( $value ),
			self::NULL => $value === Null,
			self::NUMERIC => is_numeric( $value ) || Number::isNumeric( $value ),
			self::OBJECT => is_object( $value ),
			self::RESOURCE => is_resource( $value ),
			self::SCALAR => is_scalar( $value ),
			self::STRING => is_string( $value ),
			self::OCTAL,
			self::INTEGER_OCTAL => Number::isOctal( $value ),
