...
class Template implements TemplateInterface
{
	 ...
	 * @return String
	 */
	public function clean( String $parsed, Array $except = [] ): String
	{
		// Default characters will replace.
		$chars = [ "\\#", "\\:", "\\;", "\\\"", "\\'", "\\=", "\\@", "\\[", "\\]", "\\{", "\\}", "\\<", "\\>" ];
		
		// Mapping characters.
		foreach( $except As $char )
		{
			// If caharacter exists.
		...
		return( RegExp\RegExp::replace( f( "/\\\({implode(+)})/", [ "|", $chars ] ), $parsed, "$1" ) );
	}
	 ...
	 * @return String
	 */
	public function parse( ? String $template = Null ): String
	{
		if( $template === Null )
		{
			if( $this->iteration <= 0 )
