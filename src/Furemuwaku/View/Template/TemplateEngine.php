...
class TemplateEngine implements TemplateEngineInterface
{
	...
	/*
	 * Construct method of class TemplateEngine.
	 *
	...
	private function assertClass( String $class ): Void
	{
		...
		{
			if( in_array( $class, $this->syntax->default ) ) return;
			if( in_array( $class, $this->syntax->default ) ) return;
	...
	public function handle( Data\DataInterface $match, String $view, Array $split, Int $length ): Data\DataInterface
	{
		...
		$syntax->result = f( "{}{}", $syntax->indent->value, $syntax->result );
	...
	public function unclean( String $content, Array $except = [] ): String
	{
		...
		return( preg_replace_callback( f( "/(?<!\\\)({implode(+)})/", [ "|", $chars ] ), fn( Array $m ) => "\\{$m[0]}", $content ) );
	}
